...
public class IceyeProductReader extends SARReader {
    ...
    private static final Logger LOG = Logger.getLogger(IceyeProductReader.class.getName());

    private AtomicBoolean isTiff = new AtomicBoolean();
    private AtomicBoolean isCog = new AtomicBoolean();
    private ProductReader reader;

    private Pattern legacyPattern = Pattern.compile("iceye_\\S*_\\D*_\\D*_\\d*_\\d*t\\d*\\.\\S*");
    private Pattern newPattern = Pattern.compile("iceye_\\S*_\\d*t\\d*z_\\d*_\\S*_\\D*_\\D*\\.\\S*");
    ...
    protected Product readProductNodesImpl() {
        LOG.info("----------------------- MAIN READER SELECTION ------------------");
        try {
            ...
            if (fileName.startsWith(IceyeXConstants.ICEYE_FILE_PREFIX.toLowerCase())) {
                ...
                if (fileName.endsWith(".h5")) {
                    ...
                    isTiff.set(true);
                    if (legacyPattern.matcher(fileName).matches()) {
                        isCog.set(false);
                        reader = new IceyeGRDProductReader(getReaderPlugIn());
                    } else if (newPattern.matcher(fileName).matches()) {
                        isCog.set(true);
                        if (fileName.contains("slc")) {
                            LOG.info("----------------------- SLC READER  ------------------");
                            reader = new IceyeSLCCogProductReader(getReaderPlugIn());
                        } else {
                            reader = new IceyeGRDCogProductReader(getReaderPlugIn());
                        }
                    }
                }
            }
    ...
    protected void readBandRasterDataImpl(int sourceOffsetX, int sourceOffsetY, int sourceWidth, int sourceHeight,
        ...
        if (isTiff.get()) {
            if (isCog.get()) {
                ((IceyeGRDCogProductReader) reader).callReadBandRasterData(sourceOffsetX, sourceOffsetY, sourceWidth, sourceHeight,
